...
def _top3_drawdowns(values: pd.Series):
    try:
        ...
        return [{k: (None if isinstance(v, float) and np.isnan(v) else v)
                 for k, v in r.items()} for r in top3]
    except Exception:
        return []
...
def _metrics(values: pd.Series, bench: pd.Series = None) -> dict:
    try:
        ...
        corr_ret, corr_dd = None, None
        if bench is not None:
            try:
                br    = bench.pct_change().fillna(0)
                bdd   = (bench - bench.cummax()) / bench.cummax()
                comm  = ret.index.intersection(br.index)
                commdd = dd_s.index.intersection(bdd.index)
                if len(comm) > 30:
                    corr_ret = _s(float(ret.loc[comm].corr(br.loc[comm])))
                if len(commdd) > 30:
                    corr_dd = _s(float(dd_s.loc[commdd].corr(bdd.loc[commdd])))
            except Exception:
                pass

        return {
            "cagr": _s(cagr), "total_return": _s(tr),
            "ytd": ytd,
...
# ── Holdings time series (top N assets by weight, sampled) ────────────────────
def _holdings_ts(pw: pd.DataFrame, top_n=10, max_pts=600) -> dict:
    """
    Returns:
      - ts: list of {date, asset1, asset2, ...} sampled rows
      - assets: list of asset names to show (top N by avg weight)
    """
    try:
...
def _current_holdings(pw: pd.DataFrame, tw: pd.DataFrame) -> list:
    """Latest actual weights for all assets with non-zero position."""
    try:
...
def _next_rebalance(pw: pd.DataFrame, tw: pd.DataFrame, date_index: list) -> list:
    """
    Compare last target weights to current actual weights.
    Show BUY/SELL/INCREASE/DECREASE/HOLD/EXIT for each position.
    """
    try:
        ...
        for a in all_assets:
            ...
            rows.append({
                "symbol":         a,
                "current_weight": _s(curr),
                "target_weight":  _s(tgt),
                "delta":          _s(delta),
                "action":         action,
            })
...
def _turnover_ts(turnover: pd.Series, max_pts=400) -> list:
    try:
        ...
        for i, (idx, v) in enumerate(turnover.items()):
            if i % step != 0 and i != len(turnover) - 1:
                continue
            rows.append({"date": str(idx.date()), "turnover": _s(float(v))})
        return rows
    except Exception:
        return []

# ── Regime time series ────────────────────────────────────────────────────────
def _regime_ts(regime: pd.Series, pv: pd.Series, max_pts=600) -> list:
    try:
        step = max(1, len(pv) // max_pts)
        rows = []
        for i, idx in enumerate(pv.index):
            if i % step != 0 and i != len(pv) - 1:
                continue
            rv = regime.get(idx, None) if hasattr(regime, 'get') else None
            rows.append({
                "date":   str(idx.date()),
                "value":  _s(float(pv.loc[idx])),
                "regime": _s(float(rv)) if rv is not None else None,
            })
        return rows
    except Exception:
        return []

# ── Historical positions (monthly snapshots) ──────────────────────────────────
...
def _rebal_log(tw: pd.DataFrame, pv: pd.Series) -> list:
    """
    For each rebalancing date, show the target weights as a trade event.
    """
    try:
        rows = []
...
def compute() -> dict:
    ...
    # ── Benchmark (SPY) aligned to portfolio dates ────────────────────────────
    ...
    # ── Regime series ─────────────────────────────────────────────────────────
    regime_daily = market_regime.reindex(pv.index).ffill().fillna(1)
...
class handler(BaseHTTPRequestHandler):
    def do_GET(self):
        try:
            ...
            body   = json.dumps(_cache["data"], cls=_Enc)
            status = 200
