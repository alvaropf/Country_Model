...
interface Props {
  ...
  weighting: Weighting;
  onToggleWeighting: (w: Weighting) => void;
  onRefresh: () => void;
}
...
export default function Dashboard({ data, cacheAge, weighting, onToggleWeighting, onRefresh }: Props) {
  ...
  const changes = data.next_rebalance.filter(h => Math.abs(h.delta || 0) > 0.01 ||
  ...
  return (
    <div className="min-h-screen" style={{ background: "#0a0e1a" }}>
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-gray-400">
                {data.config.start_date} → {data.config.end_date} · {data.config.rebalance} · Top {data.config.percentile_rank} · {data.config.n_assets} assets
              </p>
            </div>
          </div>
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="flex rounded-lg border border-gray-700 overflow-hidden">
              {(["ATR", "EW"] as Weighting[]).map(w => (
                <button key={w} onClick={() => onToggleWeighting(w)}
      ...
      <main className="max-w-[1600px] mx-auto px-6 py-6 space-y-6">
        ...
        {tab === "overview" && (
          <>
            <Card title={`Portfolio Performance vs SPY — ${weighting === "ATR" ? "ATR Weighted" : "Equal Weighted"} (Log Scale)`}>
            ...
            <Card title="Monthly Returns">
              <MonthlyHeatmap
                strategy={data.monthly_returns.strategy}
                benchmark={data.monthly_returns.benchmark}
                relative={data.monthly_returns.relative}
              />
            </Card>
          </>
        )}
        ...
        {tab === "rebalance" && (
          <>
            {changes.length > 0 ? (
              ...
              <Card><p className="text-gray-400 text-sm text-center py-4">✅ Portfolio is aligned — no changes for next rebalance.</p></Card>
            )}
